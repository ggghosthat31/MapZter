...
function process_app() {
    local app_path=$1
    local app_out_directory="../../out/win/$app_name/"
    
    echo "$app_path $app_out_directory"

    dotnet publish $app_path -c Release --framework=net9.0 --runtime=win-x64 --self-contained -p PublishSingleFile=True -o=$app_out_directory
}
